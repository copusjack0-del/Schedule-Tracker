/* Pocket Schedule+Pay - paste-only version */
...
let state = {
  events: load()?.events || [],
  pay: load()?.pay || { rate: 0, otThreshold: 40, otMultiplier: 1.5, deductionPct: 0, lastPayday: '' },
  focusDate: new Date(),
  deferredPrompt: null,
  imports: []
};
...
function renderDays(){
  ...
  for(let i=0;i<7;i++){
    const day=addDays(start,i);
    const evs=state.events.filter(e=>e.date===ymd(day)).sort((a,b)=>(a.start||'24:00').localeCompare(b.start||'24:00'));
...
function renderEvent(ev){
  ...
  const loc=tpl.querySelector('.location'), notes=tpl.querySelector('.notes');
  ...
  tpl.querySelector('.edit').onclick = ()=>openDialog(ev);
  return article;
}
function formatTimeRange(s,e){
  if(!s&&!e) return '';
  const fmt=t=>{ if(!t) return ''; const [h,m]=t.split(':').map(Number); const d=new Date(); d.setHours(h,m||0,0,0); return d.toLocaleTimeString([], {hour:'numeric', minute:'2-digit'}); };
...
function openDialog(ev=null){
  ...
  renderColorChoices();
  const sel=$('#color').value; $$('.colors > button').forEach(b=>{ if(b.style.background===sel) b.classList.add('selected'); });
  $('#eventDialog').showModal();
}
function upsertEventFromForm(e){
  e.preventDefault();
  const id=$('#eventId').value || crypto.randomUUID();
  const ev={ id, title: $('#title').value.trim(), date: $('#date').value,
...
/* Export/Import */
function exportJSON(){ const blob=new Blob([JSON.stringify(state.events,null,2)],{type:'application/json'}); triggerDownload(blob,`pocket-schedule-${Date.now()}.json`); }
function exportICS(){ const ics=toICS(state.events); const blob=new Blob([ics],{type:'text/calendar'}); triggerDownload(blob,`pocket-schedule-${Date.now()}.ics`); }
...
function toICS(events){
  ...
  const dtLocal=(date,time='00:00')=>{ const [h,m]=(time||'00:00').split(':').map(Number); const d=new Date(`${date}T${time||'00:00'}:00`); const pad=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}T${pad(h||0)}${pad(m||0)}00`; };
  const out=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Pocket Schedule+Pay//EN'];
  events.forEach(ev=>{ out.push('BEGIN:VEVENT',`UID:${ev.id}`,`DTSTAMP:${dtLocal(ev.date,ev.start||'00:00')}`,`DTSTART:${dtLocal(ev.date,ev.start||'00:00')}`, ev.end?`DTEND:${dtLocal(ev.date,ev.end)}`:'',`SUMMARY:${esc(ev.title)}`, ev.location?`LOCATION:${esc(ev.location)}`:'', ev.notes?`DESCRIPTION:${esc(ev.notes)}`:'','END:VEVENT'); });
  out.push('END:VCALENDAR'); return out.filter(Boolean).join('\r\n');
}
function importJSON(file){
  const r=new FileReader();
  r.onload=()=>{ try{ const data=JSON.parse(r.result); if(!Array.isArray(data)) throw new Error('Invalid file');
...
/* Pay calc */
function hoursBetween(start,end){ if(!start||!end) return 0; const [h1,m1]=start.split(':').map(Number), [h2,m2]=end.split(':').map(Number); let t=(h2*60+m2)-(h1*60+m1); if(t<0) t+=24*60; return t/60; }
function groupHoursByWeek(events){
  const weeks=new Map();
  events.forEach(e=>{ const hrs=hoursBetween(e.start,e.end); const wk=ymd(startOfWeek(e.date)); const obj=weeks.get(wk)||{hours:0}; obj.hours+=hrs; weeks.set(wk,obj); });
  const res=new Map();
  for(const [wk,info] of weeks){ const thr=state.pay.otThreshold||40; const reg=Math.min(info.hours,thr); const ot=Math.max(0,info.hours-thr); res.set(wk,{regular:reg, ot, total:info.hours}); }
  return res;
}
function currentPayPeriod(now=new Date()){
  const start=new Date(state.pay.lastPayday || new Date()); start.setHours(0,0,0,0);
  const dayMs=86400000; const n=new Date(now); n.setHours(0,0,0,0);
  let s=new Date(start);
  while (s.getTime()+14*dayMs <= n.getTime()) s=new Date(s.getTime()+14*dayMs);
...
function recalcPay(){
  ...
  const evs=state.events.filter(e=>{ const d=new Date(e.date+'T00:00:00'); return d>=period.start && d<=period.end; });
  const weekly=groupHoursByWeek(evs);
  let regH=0, otH=0, totalH=0; for(const v of weekly.values()){ regH+=v.regular; otH+=v.ot; totalH+=v.total; }
...
window.addEventListener('DOMContentLoaded', () => {
  // Tabs (FIX: target correct elements)
  ...
  $('#parsePasted').onclick = ()=>{
    ...
    renderImportPreview();
    $('#gmailStatus').textContent = `Found ${state.imports.length} shift(s).`;
  ...
  $('#commitImports').onclick = ()=>{
    ...
    state.imports.forEach(e=>{ const k=[e.title,e.date,e.start,e.end,e.location].join('|'); if(!keys.has(k)){ state.events.push({...e, id: crypto.randomUUID()}); keys.add(k);} });
    state.imports=[]; save(); render(); recalcPay();
